...
export default function Hero() {
  ...
  return (
    <section
      ref={containerRef}
      className="relative min-h-screen w-full bg-[#FAF5F2] overflow-hidden flex flex-col justify-between"
    >
      {/* Full-viewport split layout */}
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 w-full items-stretch">
        
        {/* Left Column: Narrative Content */}
        <div className="lg:col-span-6 order-2 lg:order-1 flex flex-col justify-center bg-[#FAF5F2] px-6 sm:px-12 lg:px-20 xl:px-24 py-16 lg:py-24 relative z-20">
          <motion.div
          ...
          >
            ...
            <motion.h1 
              className="font-serif text-4xl sm:text-5xl lg:text-6xl xl:text-7xl text-[#1C1C1C] font-normal leading-[1.12] mb-6"
            >
              {words.map((word, idx) => (
            ...
            >
              <a
                href="#contact"
                className="text-center font-sans font-semibold text-xs uppercase tracking-wider bg-[#1C1C1C] text-[#FAF5F2] hover:bg-[#6B6F4C] px-8 py-4 rounded-full transition-all duration-300 shadow-sm active:scale-[0.98]"
              ...
              <a
                href="#services"
                className="text-center font-sans font-semibold text-xs uppercase tracking-wider text-[#1C1C1C] hover:text-[#6B6F4C] px-8 py-4 rounded-full border border-[#D8CBB8] hover:border-[#6B6F4C]/45 hover:bg-[#EFE7D8]/30 transition-all duration-300 active:scale-[0.98]"
                style={{ minHeight: 44, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}
              >
                Explore Services
              </a>
        ...
        <div className="lg:col-span-6 order-1 lg:order-2 relative overflow-hidden min-h-[40vh] lg:min-h-screen z-10 bg-[#EFE7D8]">
          ...
          >
            ...
            {/* Vignette overlay */}
            <div className="absolute inset-0 bg-gradient-to-t from-[#1C1C1C]/15 via-transparent to-transparent pointer-events-none z-10" />
          </motion.div>
        </div>
      </div>
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-40 pointer-events-none hidden lg:block">
        <a
          href="#about"
          className="flex flex-col items-center gap-2 text-[#78716C]/60 hover:text-[#1C1C1C] transition-colors pointer-events-auto group"
